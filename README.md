...
* ### sed 사용하기
  ...
  * sed -e 사용하기
    ...
    **쉘 스크립트에서 직접 해보기**
    ...
    -n -e '1p' -e '5p'             ---> 첫번째 줄과 다섯번째 줄만 출력\
    -n -e '1,3d' -e '1,$p'         --->전체에서 1~3줄을 제외하고 출력\
    -n -e 's/1/a/g' -e '1,$p'      --->모든 1를 a로 바꾸고 출력\
    -n -e '/1234/a\aaa' -e '1,$p'  --->모든 1234문자열 뒤에 aaa 추가하고 출력\
    -n -e '/1234/i\aaa' -e '1,$p'  --->모든 1234문자열 앞에 aaa 추가하고 출력
